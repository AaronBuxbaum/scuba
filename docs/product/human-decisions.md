...
- **Starting policy:** the full per-diver price (trip price, or the course's priced pair) is asked
  for at booking through one hosted Stripe Checkout on the shop's connected account; no deposit
  tier, no tax lines, no platform fee, no automated cancellation/refund window. Refunds stay
  staff-initiated from the diver's payment context.
- **Safety boundary:** payment never gates the capacity-safe booking transaction — a diver who
  ...
  `payment_due` blocker where the trip requires payment. Paid state is recorded only from Stripe's
  own webhook or API responses, never from a return URL.
- **Opt-out:** leave the trip unpriced or the Stripe account unconnected and the public flow stays
  book-now-pay-later.
- **Must verify (H-07):** deposit vs. full price, cancellation/refund windows and who runs them,
  tax treatment, any platform fee, and whether unpaid bookings should auto-expire.
